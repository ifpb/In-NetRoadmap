...
def writeactionrule(writer, ranges, action, result, tree_id):
    command = f"table_add {tree_id}_classify_exact {action} "
...
def generate_table(rf_data: dict, output_path: str):
    ...
    with open(output_path, "w") as f:
        for t_idx in range(1, n_trees + 1):
            tree_key = f"t{t_idx}"

            tree_data = rf_data[tree_key]
            features = copy.deepcopy(tree_data["thresholds"])
            regras_list = tree_data["regras"]
            ...
            for i in range(len(rules)):
                ...
                writeactionrule(f, ranges, f"set_result", ind, tree_key)
            ...
            for idx, fea in enumerate(features.keys()):
                ...
                for i in range(len(features[fea]) - 1):
                    writefeatureXrule(
                        f,
                        features[fea][i : i + 2],
                        f"{tree_key}_feature{idx+1}_exact",
